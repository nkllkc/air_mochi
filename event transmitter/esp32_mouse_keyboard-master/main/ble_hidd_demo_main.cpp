...
/*void console_uart_parse_command(uint8_t character, struct cmdBuf * cmdBuffer, mouse_command_t * mouseCmd)
{
    switch (cmdBuffer->state) {
        case CMDSTATE_IDLE:
            ESP_LOGI(CONSOLE_UART_TAG,"First character was ignored it was '%c'", character);
            cmdBuffer->bufferLength=0;
            cmdBuffer->state=GETLAT;
            mouseCmd->x=0;
            mouseCmd->y=0;
            break;
        case CMDSTATE_GET_RAW:
            cmdBuffer->buf[cmdBuffer->bufferLength]=character;
            ESP_LOGI(CONSOLE_UART_TAG,"New character for the command is '%c'", character);
            cmdBuffer->bufferLength++;
            cmdBuffer->expectedBytes--;
            if (!cmdBuffer->expectedBytes){
                    if(HID_kbdmousejoystick_rawMouse(&(cmdBuffer->buf[0]),4) != ESP_OK)
                    {
        ...
        case GETLAT:
            cmdBuffer->buf[cmdBuffer->bufferLength]=character;
            ESP_LOGI(CONSOLE_UART_TAG,"New character for LAT is '%c'", character);
            cmdBuffer->bufferLength++;
            if(character == '/'){
                cmdBuffer->state=GETLONG;
            }else{
                int new_digit = 0;
                ESP_LOGI(CONSOLE_UART_TAG,"New character for LAT is '%c'", character);
                new_digit = (int)character - 48;
                ESP_LOGI(CONSOLE_UART_TAG,"New number for LAT is '%d'", new_digit);
                int lat = (mouseCmd->x)*10 + new_digit;
                ESP_LOGI(CONSOLE_UART_TAG,"The latitude is '%d'", lat);
                mouseCmd->x = lat;
            }
            break;
        case GETLONG:
            cmdBuffer->buf[cmdBuffer->bufferLength]=character;
                ESP_LOGI(CONSOLE_UART_TAG,"New character for LONG is '%c'", character);
                cmdBuffer->bufferLength++;
                if(character == '/'){
                    mouseCmd->buttons = 0;
                    mouseCmd->wheel = 0;
                    ESP_LOGI(CONSOLE_UART_TAG,"Before sending the command: x is '%d' and y is '%d'", mouseCmd->x, mouseCmd->y);
                    cmdBuffer->state=CMDEXEC;

                }else{
                    int new_digit = (int)character - 48;
                    ESP_LOGI(CONSOLE_UART_TAG,"New number for LONG is '%d'", new_digit);
                    int longtitude = (mouseCmd->y)*10 + new_digit;
                    ESP_LOGI(CONSOLE_UART_TAG,"The longtitude is '%d'", longtitude);
                    mouseCmd->y = longtitude;
                }
            break;
...
/*void uart_console(void *pvParameters)
{
    ...
    while(1)
    {
        ...
        ESP_LOGI(CONSOLE_UART_TAG,"Not connected, ignoring '%d'", character);

		if(HID_kbdmousejoystick_isConnected() == 0) {
			ESP_LOGI(CONSOLE_UART_TAG,"Not connected, ignoring '%c'", character);
		} else {
//		    console_uart_parse_command(character, &cmdBuffer, &mouseCmd);
//            if(cmdBuffer.state==CMDEXEC){
//                xQueueSend(mouse_q,(void *)&mouseCmd, (TickType_t) 0);
//                cmdBuffer.state=CMDSTATE_IDLE;
//            }
		   //
			...
			switch (character) {
                ...
                case 'k':
                    ...
                    mouseCmd.wheel = 0;
                    xQueueSend(mouse_q, (void *) &mouseCmd, (TickType_t) 0);
                    ESP_LOGI(CONSOLE_UART_TAG, "mouse: k - New command added");
                    break;
                default:
...
void uart_console(void *pvParameters)
{
    ...
    {
        // read single byte
        uart_read_bytes(CONSOLE_UART_NUM, (uint8_t*) &latitude, 1, portMAX_DELAY);
        ESP_LOGI(CONSOLE_UART_TAG,"x: '%d'", latitude);
        uart_read_bytes(CONSOLE_UART_NUM, (uint8_t*) &longtitude, 1, portMAX_DELAY);
        ESP_LOGI(CONSOLE_UART_TAG,"y: '%d'", longtitude);
        uart_read_bytes(CONSOLE_UART_NUM, (uint8_t*) &click_button, 1, portMAX_DELAY);
        ESP_LOGI(CONSOLE_UART_TAG,"click: '%d'", click_button);
        if(HID_kbdmousejoystick_isConnected() == 0) {
            // ESP_LOGI(CONSOLE_UART_TAG,"Not connected, ignoring '%c'", character);
        } else {
//		    console_uart_parse_command(character, &cmdBuffer, &mouseCmd);
