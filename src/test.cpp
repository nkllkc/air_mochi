...
int main(int argc, char** argv)
{
    ...
    for (; p < h264_raw + file_size;) {
        ...
        char nut = (char)data[nb_start_code] & 0x1f;
        srs_human_trace("sent packet: type=%s, decode time=%d, presentation time=%d, size=%d, fps=%.2f, b[%d]=%#x(%s)",
            srs_human_flv_tag_type2string(SRS_RTMP_TYPE_VIDEO), dts, pts, size, fps, nb_start_code, (char)data[nb_start_code],
